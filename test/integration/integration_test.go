...
func (mockHttpOperations) HttpGet(client *http.Client, url string, headers map[string]string) (*transport.HttpRes, error) {
	os.MkdirAll(tempDir+"/http_get", 0o700)
...
func checkStatus(t *testing.T, ctx context.Context, cfg *compose.Config, targetID string) {
	status, err := status.GetCurrentStatus(ctx, cfg)
	if err != nil {
		t.Fatalf("GetStatus failed: %v", err)
	}
	// fmt.Printf("Current status: %+v\n", status)
	if status.TargetID != targetID {
		t.Fatalf("Current target %s does not match expected target %s", status.TargetID, targetID)
	}
}
...
func createMockConfig(t *testing.T) *cfg.Config {
...
func saveTargetsJson(t *testing.T, targets []*Target) {
	targetsStr := []string{}
...
`, formattedTimeExpiration, targetsJson)
	...
	outDir := tempDir + "/http_get/repo/"
	if err := os.MkdirAll(outDir, 0o700); err != nil && !os.IsExist(err) {
		t.Fatalf("Unable to create http_get/repo dir: %v", err)
	}

	if err := os.WriteFile(outDir+"targets.json", []byte(fullJson), 0o600); err != nil {
		t.Fatalf("Unable to write targets.json: %v", err)
	}
}

func genNewTarget(t *testing.T, version int, numberOfApps int, portOffset int, badApps bool) *Target {
	appComposeDef := `
services:
  srvs-01:
...
`
	if badApps {
		appComposeDef = strings.Replace(appComposeDef, "sh -c", "badcommand", -1)
	}
	apps := []*f.App{}
